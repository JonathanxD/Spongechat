...
final class ConfigurationSection
{
	...
	private final List<String> parser = new ArrayList<String>();
	...
	protected ConfigurationSection sectionTransform(final String content)
	{
		final HashMap<String, String> kv = new HashMap<>();// HashMap to Store keys and Values to send to function configureYAML
		for (int x = 0; x != content.split("\n").length; ++x)
		{ // Split line-by-line
			final String keyValue = content.split("\n")[x]; // Get current line
			final String key = keyValue.split(":")[0]; // get the key
			final String value = keyValue.split(":").length != 1 ? keyValue.split(":")[1] : ""; // get the value (if exists)
	...
	protected <T> T configureYAML(final int doTask, final Object... args)
	{
		...
		if (doTask == TASK_FORM_SECTION || doTask == TASK_SECTION_CHECK || doTask == TASK_SECTION_TRANSFORM || doTask == TASK_DIRECTIVE_CHECK)
		{
			...
			if (doTask == TASK_FORM_SECTION)
				parser.clear(); // Clear parser (Configuration class)
			if (doTask == TASK_SECTION_TRANSFORM)
				tmp = ""; // Create clear (non-null) temporary string
			if (doTask == TASK_SECTION_TRANSFORM)
				map = (HashMap<String, String>) args[0]; // Get unformatted key/value map from args

			final String split[] = sectionTransform.split("\n"); // Split line by line of sectionTransform

			for (int x = 0; x < split.length; ++x)
			{ // Loop lines

				String current = split[x]; // Get current line
				final String next = x + 1 != split.length ? split[x + 1] : null; // Get next line

				final HashMap<String, String> directiveToCreate = new HashMap<>(); // Map to store directives to create (safe process (prevent directive be inserted in current line, and be inserted in
																					// last line))

				if (doTask == TASK_SECTION_TRANSFORM)
				{ // Value set fix
					final Iterator<Map.Entry<String, String>> vs = map.entrySet().iterator(); // Make a iterator of map
					while (vs.hasNext())
					{ // Check if has next element
						final Map.Entry<String, String> ms = vs.next(); // Get the element
						if (diretiveExists(ms.getKey()) == null && !ms.getKey().contains("."))
						{ // Check directive not exists
					...
					if (!hasIdent(current) && !hasIdent(next) && current.split(":").length > 1)
					{ // Check if is a directive
						final String key = current.split(":")[0]; // Get directive key
				...
				if (doTask == TASK_SECTION_TRANSFORM || doTask == TASK_DIRECTIVE_CHECK)
				{ // Check directives and sections
					if ((!hasIdent(current) && (next == null || !hasIdent(next))))
					{ // Check if element is directive
						if (doTask == TASK_DIRECTIVE_CHECK)
						{ // Directives

							if (args[0].equals(current.split(":")[0]))
							{ // Check if arg directive equal the current directive
				...
				final HashMap<Integer, String> indexes = new HashMap<>(); // Map to put next elements (process - one-by-one)
				final HashMap<Integer, String> indexies = new HashMap<>(); // Map to put next elements (store - all)
				final HashMap<String, String> sectionToCreate = new HashMap<>(); // Map to store sections to create (safe process (prevent section be inserted in current line, and be inserted in last
																					// line))

				final ArrayList<String> xcurr = new ArrayList<String>(); // Key String Map

				if (!hasIdent(current) && current.split(":").length == 1)
				{ // Check if is a section Main
					...
					try
					{
						try
						{
							...
							{
								final String nextCurr = split[x + z];
								indexies.put(getIdentsInt(nextCurr) + 1, nextCurr);
								if (nextCurr.split(":").length != 1)
								{
									final String key = getHierarchy(indexies, nextCurr, main) + "." + nextCurr.replace(" ", "").split(":")[0];
									xcurr.add(key);
								}
						...
						{ // Go to next element
							...
							{// Check if is a section (= 1) or a directive (!= 1)
								String value = nextCurr.split(":").length != 1 ? nextCurr.split(":")[1] : null; // Get value
								final String key = hasIdent(nextCurr) ? getHierarchy(indexes, nextCurr, main) + "." + nextCurr.replace(" ", "").split(":")[0] : removeIdents(nextCurr).split(":")[0]; // Get
																																																		// key
																																																		// unformatted
																																																		// (like:
																																																		// One.Two.Three)
								...
								if (removeIdents(key).startsWith("[") && removeIdents(key).endsWith("]"))
								{ // Check if is Array[]
									// Is Array
									// Will be processed by Configuration class.
								}
								if (doTask == TASK_SECTION_TRANSFORM)
								{ // Transform section (key/value map to formatted YAML)
									if (map.containsKey(key))
									{ // Check if map contains that key (exists)
										value = map.get(key); // Set process, old value (value) to new value (map.get)
										if (!isNull(value))
										{ // Check is not null value
								...
								if (hasIdent(nextCurr))
								{
									...
									if (doTask == TASK_FORM_SECTION)
									{ // Formatted YAML to key/value map (unformatted)
										if (!isNull(value))
										{
							...
							{
								if (doTask == TASK_SECTION_TRANSFORM)
								{ // Process section create (HARD WORK :P (Know that, bugs aren't friends!))
									tmp += nextCurr + "\n"; // Append current to new line in temporary variable (document)
									final Iterator<Map.Entry<String, String>> vs = map.entrySet().iterator(); // Make a iterator of map
									while (vs.hasNext())
									{ // Check if has next element
										final Map.Entry<String, String> ms = vs.next(); // Get the element
										if ((diretiveExists(ms.getKey()) != null && diretiveExists(ms.getKey())) || sectionExists(ms.getKey()))
										{ // Check if directive or section exists
										}
										else
										{
											if (diretiveExists(ms.getKey()) != null && !diretiveExists(ms.getKey()))
											{ // Check directive not exists
												// MARK: Removed! Moved to line: 61
											}
											else if (!sectionExists(ms.getKey()))
											{ // Check if section not exists
												final String key = getHierarchy(indexes, nextCurr, main) + "." + nextCurr.replace(" ", "").split(":")[0]; // Get key
												if (findSection(xcurr, ms.getKey()))
												{ // Find the section, true if exists
													if (!findSectionEqual(xcurr, ms.getKey(), key))
													{ // Check equal current element (prevent bugs)
														continue;
													}

													if (ms.getKey().startsWith(key))
													{ // Check if equal current... ? Again? Ok
														final String kay = getIdents(nextCurr) + ident + ms.getKey().split(key + "\\.")[1];// Set key to create
														String laast = null; // Last key
														for (int xx = 0; xx < kay.split("\\.").length; ++xx)
														{ // Loop Elements (Like: One.Two.Three (0. One, 1. Two, 2. Three))
				...
				if (doTask == TASK_SECTION_TRANSFORM)
				{
					final Iterator<Map.Entry<String, String>> vs = directiveToCreate.entrySet().iterator();
					while (vs.hasNext())
					{
						final Map.Entry<String, String> curr = vs.next();
						tmp += curr.getKey() + ":" + curr.getValue() + "\n";
					}
					final Iterator<Map.Entry<String, String>> ds = sectionToCreate.entrySet().iterator();
					while (ds.hasNext())
					{
						final Map.Entry<String, String> curr = ds.next();
						final String kay = curr.getKey();
						String laast = null;
						for (int xx = 0; xx < kay.split("\\.").length; ++xx)
						{
			...
			if (doTask == TASK_FORM_SECTION)
				return (T) this;
			if (doTask == TASK_SECTION_CHECK /* || doTask == TASK_DIRETIVE_CHECK */)
	...
	private boolean findSection(final ArrayList<String> sArray, final String find)
	{
		for (String s : sArray)
		{
	...
	private boolean findSectionEqual(final ArrayList<String> sArray, final String find, final String curr)
	{
		for (String s : sArray)
		{
			s = s.substring(0, s.lastIndexOf("."));
	...
	private String moreSimilar(final List<String> list, final String similarCheck)
	{
		String c = list.get(0);
		final TreeMap<Integer, String> points = new TreeMap<Integer, String>();
	...
	private String getHierarchy(final HashMap<Integer, String> hierarchy, String element, final String main)
	{
		String result = "";
	...
	{
		final Object result = configureYAML(TASK_SECTION_CHECK, section);
		return (result != null && (Boolean) result == Boolean.TRUE ? true : false);
	}

	private Boolean diretiveExists(final String diretive)
	{
		final Object result = configureYAML(TASK_DIRECTIVE_CHECK, diretive);
		if (result == null)
			return null;
		return (result != null && (Boolean) result == Boolean.TRUE ? true : false);
	}

	protected void setNewSectionTransform(final String transform)
	{
		sectionTransform = transform;
	}

	protected void setNewSectionTransform(final List<String> transform)
	{
		sectionTransform = "";
		for (final String s : transform)
		{
			sectionTransform += s + "\n";
		}
	...
	private String getIdents(final String s)
	{
		if (s == null)
			return "";
		int local = 0;
	...
	private int getIdentsInt(final String s)
	{
		if (s == null)
			return 0;
		int local = 0;
	...
	private String removeIdents(final String s)
	{
		if (s == null)
			return s;
		int local = 0;
	...
	private boolean hasIdent(final String s)
	{
		if (s == null)
			return false;
